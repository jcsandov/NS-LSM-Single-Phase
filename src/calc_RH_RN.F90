...
!call outputD3_real(dcsimenos(:,:,:),'dcsimenosFO')
!call outputD3_real(dcsimenos(:,:,:),'dcsimasFO')
...
!ahora calculo la funcion lado derechp
i_sta = il + igp
j_sta = jl + jgp
k_sta = kl + kgp
...
do k = k_sta, k_end
        ...
        if(signo_RN(i,j,k) > zero) then
                normagradf = normagradfmas
        else if (signo_RN(i,j,k) < zero ) then
                normagradf = normagradfmenos
        else
                normagradf = one
        end if

        rightH_RN(i,j,k) = signo_RN(i,j,k) * (normagradf - one)     
        !rightH_RN(i,j,k) = normagradf
end do
end do
end do

end subroutine calc_RH_RN
