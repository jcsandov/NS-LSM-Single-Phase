...
real (kind = rdf), dimension(1:nms) :: ttvec, tsvec ! Tt and Ts vectors of the SVD system
...
! pressure LSQM system

real (kind = rdf), dimension(4,4) :: t_matrix_pressure, P_LSQM_INV
real (kind = rdf), dimension(4)   :: bvec 
real (kind = rdf), dimension(4)   :: bvec_x , bvec_y , bvec_z
real (kind = rdf), dimension(4)   :: grad_p_norm_coeffs
real (kind = rdf), dimension(4)   :: dp_dx_coeffs, dp_dy_coeffs, dp_dz_coeffs
real (kind = rdf), dimension(3)   :: grad_p_fs_lsqm
real (kind = rdf)  :: dp_dx , dp_dy , dp_dz
real (kind = rdf)  :: grad_p_norm
real (kind = rdf)  :: grad_p_norm_fs_lsqm
   ...
   if( nWaterNeighbours > one_half ) then
      ...
      do ii = max( ista , i-sweep_lsqm ) , min( iend , i+sweep_lsqm )
        ...
        do icoc = 1,3
           ...
           t_matrix_system(2*nms+icoc,2) =    t_matrix_system(2*nms+icoc,2) &
                                            + alpha_local(icoc) * alpha_local(1)
           t_matrix_system(2*nms+icoc,3) =    t_matrix_system(2*nms+icoc,3) &
                                            + alpha_local(icoc) * alpha_local(2)
           t_matrix_system(2*nms+icoc,4) =    t_matrix_system(2*nms+icoc,4) &
                                            + alpha_local(icoc) * alpha_local(3)

           t_matrix_system(2*nms+icoc,18) =   t_matrix_system(2*nms+icoc,18) &
                                            + alpha_local(icoc) * alpha_local(1)
           t_matrix_system(2*nms+icoc,19) =   t_matrix_system(2*nms+icoc,19) &
                                            + alpha_local(icoc) * alpha_local(2)
           t_matrix_system(2*nms+icoc,20) =   t_matrix_system(2*nms+icoc,20) &
                                            + alpha_local(icoc) * alpha_local(3)

           t_matrix_system(2*nms+icoc,34) =   t_matrix_system(2*nms+icoc,34) &
                                            + alpha_local(icoc) * alpha_local(1)
           t_matrix_system(2*nms+icoc,35) =   t_matrix_system(2*nms+icoc,35) &
                                            + alpha_local(icoc) * alpha_local(2)
           t_matrix_system(2*nms+icoc,36) =   t_matrix_system(2*nms+icoc,36) &
                                            + alpha_local(icoc) * alpha_local(3)
        ...
        ! 
        t_matrix_pressure(1,1) = t_matrix_pressure(1,1) + one         * one
        t_matrix_pressure(1,2) = t_matrix_pressure(1,2) + one         * x(ii,jj,kk)
        t_matrix_pressure(1,3) = t_matrix_pressure(1,3) + one         * y(ii,jj,kk)
        t_matrix_pressure(1,4) = t_matrix_pressure(1,4) + one         * z(ii,jj,kk)

        t_matrix_pressure(2,1) = t_matrix_pressure(2,1) + x(ii,jj,kk) * one 
        t_matrix_pressure(2,2) = t_matrix_pressure(2,2) + x(ii,jj,kk) * x(ii,jj,kk) 
        t_matrix_pressure(2,3) = t_matrix_pressure(2,3) + x(ii,jj,kk) * y(ii,jj,kk) 
        t_matrix_pressure(2,4) = t_matrix_pressure(2,4) + x(ii,jj,kk) * z(ii,jj,kk) 

        t_matrix_pressure(3,1) = t_matrix_pressure(3,1) + y(ii,jj,kk) * one 
        t_matrix_pressure(3,2) = t_matrix_pressure(3,2) + y(ii,jj,kk) * x(ii,jj,kk) 
        t_matrix_pressure(3,3) = t_matrix_pressure(3,3) + y(ii,jj,kk) * y(ii,jj,kk) 
        t_matrix_pressure(3,4) = t_matrix_pressure(3,4) + y(ii,jj,kk) * z(ii,jj,kk) 

        t_matrix_pressure(4,1) = t_matrix_pressure(4,1) + z(ii,jj,kk) * one 
        t_matrix_pressure(4,2) = t_matrix_pressure(4,2) + z(ii,jj,kk) * x(ii,jj,kk) 
        t_matrix_pressure(4,3) = t_matrix_pressure(4,3) + z(ii,jj,kk) * y(ii,jj,kk) 
        t_matrix_pressure(4,4) = t_matrix_pressure(4,4) + z(ii,jj,kk) * z(ii,jj,kk) 

        call pressure_gradient( ii , jj , kk , dp_dx , dp_dy , dp_dz , exsign)
        !call pressure_gradient2( ii , jj , kk , dp_dx , dp_dy , dp_dz , exsign)

        ! || ∇p_ii,jj,kk ||
        !grad_p_norm = sqrt( dp_dx * dp_dx + dp_dy * dp_dy + dp_dz * dp_dz )

        grad_p_norm = rsign(ii,jj,kk) * q(1,ii,jj,kk)/ ( phi(ii,jj,kk) + eps_sims )

        bvec_x(1) = bvec_x(1) + exsign * dp_dx * one
        bvec_x(2) = bvec_x(2) + exsign * dp_dx * x(ii,jj,kk) 
        bvec_x(3) = bvec_x(3) + exsign * dp_dx * y(ii,jj,kk)
        bvec_x(4) = bvec_x(4) + exsign * dp_dx * z(ii,jj,kk)

        bvec_y(1) = bvec_y(1) + exsign * dp_dy * one
        bvec_y(2) = bvec_y(2) + exsign * dp_dy * x(ii,jj,kk) 
        bvec_y(3) = bvec_y(3) + exsign * dp_dy * y(ii,jj,kk)
        bvec_y(4) = bvec_y(4) + exsign * dp_dy * z(ii,jj,kk)

        bvec_z(1) = bvec_z(1) + exsign * dp_dz * one
        bvec_z(2) = bvec_z(2) + exsign * dp_dz * x(ii,jj,kk) 
        bvec_z(3) = bvec_z(3) + exsign * dp_dz * y(ii,jj,kk)
        bvec_z(4) = bvec_z(4) + exsign * dp_dz * z(ii,jj,kk)

        bvec(1)   = bvec(1)   + rsign(ii,jj,kk) * exsign * grad_p_norm * one
        bvec(2)   = bvec(2)   + rsign(ii,jj,kk) * exsign * grad_p_norm * x(ii,jj,kk) 
        bvec(3)   = bvec(3)   + rsign(ii,jj,kk) * exsign * grad_p_norm * y(ii,jj,kk)
        bvec(4)   = bvec(4)   + rsign(ii,jj,kk) * exsign * grad_p_norm * z(ii,jj,kk)
      ...
      !----------------------------------------------------------------------
      ! 5.1 Pressure LSQM matrix inversion
      !
      !----------------------------------------------------------------------

      ! matrix inversion
      call M44INV( t_matrix_pressure , P_LSQM_INV , OK_FLAG )

      ! inversion rhs vector multiplication
      dp_dx_coeffs = matmul ( P_LSQM_INV , bvec_x ) ! ( ax , bx , cx , dx )
      dp_dy_coeffs = matmul ( P_LSQM_INV , bvec_y ) ! ( ay , by , cy , dy ) 
      dp_dz_coeffs = matmul ( P_LSQM_INV , bvec_z ) ! ( az , bz , cz , dz )

      grad_p_norm_coeffs = matmul ( P_LSQM_INV , bvec ) 


      grad_p_fs_lsqm(1)   =  dp_dx_coeffs(1)        + &  
                             dp_dx_coeffs(2) * xs   + &
                             dp_dx_coeffs(3) * ys   + &
                             dp_dx_coeffs(4) * zs  

      grad_p_fs_lsqm(2)   =  dp_dy_coeffs(1)        + &  
                             dp_dy_coeffs(2) * xs   + &
                             dp_dy_coeffs(3) * ys   + &
                             dp_dy_coeffs(4) * zs  

      grad_p_fs_lsqm(3)   =  dp_dz_coeffs(1)        + &  
                             dp_dz_coeffs(2) * xs   + &
                             dp_dz_coeffs(3) * ys   + &
                             dp_dz_coeffs(4) * zs  

      grad_p_norm_fs_lsqm =  grad_p_norm_coeffs(1)        + &  
                             grad_p_norm_coeffs(2) * xs   + &
                             grad_p_norm_coeffs(3) * ys   + &
                             grad_p_norm_coeffs(4) * zs  

      if ( grad_p_norm_fs_lsqm < 0.8 .or. grad_p_norm_fs_lsqm > 1.2 ) then
         write(*, '(A, I0, A, I0, A, I0, A, F13.8)') "i = ", i, " , j = ", j, ", k = ", k , &
         ', grad_p_norm_fs_lsqm = ', grad_p_norm_fs_lsqm     
         print *, 'bvec               = ', bvec
         print *, 'grad_p_norm_coeffs = ', grad_p_norm_coeffs

      end if

      !----------------------------------------------------------------------
      ! 5.2 Singular Value Decomposition (SVD) step
      !
...
subroutine pressure_gradient( i, j, k, dp_dx , dp_dy , dp_dz , exsign )
   ...
   integer, intent(in) :: i,j,k
   real (kind = rdf), intent(out)   :: dp_dx , dp_dy , dp_dz 
   real (kind = rdf), intent(inout) :: exsign ! flag variable. exsign = 0, the gradient is set to zero
                               !                exsign = 1, the computed gradient is returned
...
subroutine pressure_gradient2( i, j, k, dp_dx , dp_dy , dp_dz , exsign )
...
end subroutine pressure_gradient2
...
subroutine ghost_nodes_extrapolation(i,j,k, xs, ys, zs, u_fs_lsqm, du_dx_fs_lsqm, grad_p_fs_lsqm)
   ...
   real (kind = rdf), dimension(1:3), intent(in) :: u_fs_lsqm
   real (kind = rdf), dimension(1:3,1:3), intent(in) :: du_dx_fs_lsqm
   real (kind = rdf), dimension(3), intent(in) :: grad_p_fs_lsqm
   real (kind = rdf), intent(in) :: xs, ys, zs ! free-surface position closest to (i,j,k) node
