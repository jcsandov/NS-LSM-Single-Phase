...
subroutine mg_metrics
  ...
  do n = 1, ng
     ...
     ! internal nodes are computed using second-order central difference
     ! boundaries are computed using second-order upwind differentiation
     ...
     if (myback == mpi_proc_null) then
        ...
        do j = ja, jb
           ...
           xc(l) = dum * (-one * x(l+2*ic) + four * x(l+ic) - three * x(l))
           yc(l) = dum * (-one * y(l+2*ic) + four * y(l+ic) - three * y(l))
           zc(l) = dum * (-one * z(l+2*ic) + four * z(l+ic) - three * z(l))
...
!           zc(l) = dum1 * (z(l) - z(l-ic))
     ...
     if (myleft  == mpi_proc_null) then
        ...
        do i = ia, ib
           ...
           xe(l) = dum * (-one * x(l+2*ie) + four * x(l+ie) - three * x(l))
           ye(l) = dum * (-one * y(l+2*ie) + four * y(l+ie) - three * y(l))
           ze(l) = dum * (-one * z(l+2*ie) + four * z(l+ie) - three * z(l))
...
!           ze(l) = dum1 * (z(l) - z(l-ie))
     ...
     if (mydown == mpi_proc_null) then
        ...
        do i = ia, ib
           ...
           xz(l) = dum * (-one * x(l+2*iz) + four * x(l+iz) - three * x(l))
           yz(l) = dum * (-one * y(l+2*iz) + four * y(l+iz) - three * y(l))
           zz(l) = dum * (-one * z(l+2*iz) + four * z(l+iz) - three * z(l))
