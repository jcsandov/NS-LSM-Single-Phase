module global_param
  ...
  ! -------------------------
  logical :: phi_inlet_prescribed = .false.


  ! switchs to turn on/off flow solver and level-set method
  logical :: call_solver_daf 

  ! hydraulic or gravity source term mode for LSM
  logical :: hydraulic_mode

  ! if true, calculates local tau using the velocity
  logical :: dynamic_dtau

  ! Non-slip walls blanking
  logical :: non_slip_wall_blanking

  ! Values to save specific iterations in the code
  integer :: n_save_NSdebug_tsteps ! total of time steps to be saved
  integer :: current_NSdebug_tstep ! it's been rewritten every time I save a debug solution
  integer :: current_NSdebug_counter ! it's been rewritten every time I save a debug solution
  integer, dimension(20) :: save_NSdebug_tsteps = -1 ! list with tsteps to be saved (max 20)
