subroutine calc_phi_gradient_ENO2( il,iu          ,&
                                   jl,ju          ,&
                                   kl,ku          ,&
                                   igp, jgp, kgp  ,&
                                   dc, de, dz     ,&
...
do i = i_mysta , i_myend
   ...
   dphi_dcsi = dc * GetENO2Reconstruction( phiC, phiLL, phiL, phiC, phiR, phiRR , &
   ...
   dphi_deta = de * GetENO2Reconstruction( phiC, phiLL, phiL, phiC, phiR, phiRR , &
   ...
   dphi_dzet = dz * GetENO2Reconstruction( phiC, phiLL, phiL, phiC, phiR, phiRR , &
   ...
   if (nblk /= 0) then
      ...
      do nb = 1,nblk
         ...
         if( i==li_blk_ia(1,nb) .and. &
             j>=li_blk_ja(1,nb) .and. j<=li_blk_jb(1,nb) ) dphi_dcsi = zero
   
         if( i==li_blk_ib(1,nb) .and. &
             j>=li_blk_ja(1,nb) .and. j<=li_blk_jb(1,nb) ) dphi_dcsi = zero
   
         if( j==li_blk_ja(1,nb) .and. &
             i>=li_blk_ia(1,nb) .and. i<=li_blk_ib(1,nb) ) dphi_deta = zero
...
end subroutine calc_phi_gradient_ENO2
