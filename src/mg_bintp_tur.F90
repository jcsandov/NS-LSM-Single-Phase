  ...
  subroutine mg_bintp_tur
  ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ! Mass flux based interpolation method
  ...
  real (kind = rdf) :: chi, chic, fv1, rei
  ...
  if (ll /= nosend) print*, 'll /= nosend:',myid, np, ll, nosend

  ! Sends a distinct message from each task to every task.
  ! Messages can have different sizes and displacements.
  !
  call mpi_alltoallv (sbuf, sendcounts, sdispls, MPI_REAL_TYPE, &
                      rbuf, recvcounts, rdispls, MPI_REAL_TYPE, mpi_comm_world, ierr)

  ! interface information and interpolate data
  !
  ll = 0
  do np = 0, nproc - 1
     ...
     do ni = 1, donee(np)
       ...
       !l = gi_2_le_idx(i,j,k,ns)
        ...
        if (nz /= myzone) print*, 'something wrong in mg_osg_bintp'
