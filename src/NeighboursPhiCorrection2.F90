subroutine NeighboursPhiCorrection2( nnodes, ntetrahedra, PNodesList, KTetrahedraList , InterfaceNodesID, &
                                     TotalWaterVolume )
   ...
   !print *, 'C_i BEFORE Newton Raphson = ', C_i

   call NewtonRaphson (ntetrahedra, KTetrahedraList, C_InitialGuest, C_i, TotalWaterVolume)
!   call NewtonRaphson (ntetrahedra, KTetrahedraList, C_InitialGuest, C_i, TotalWaterVolume)
   ...
   do Inode = 1 , nnodes
      ...
      PhiCorrectionAux = CCorrected * PNodesList(Inode)%xiI

      ! If PhiCorrection is too far from the geometric distance ( 100% more ), then we 
      ! drop the correction and we keep the geometric distance

      !if ( abs ( PhiCorrectionAux ) >  PNodesList(I)%SDistanceFreeSurface  ) then
      !   PhiCorrectionAux = zero
      !end if
   ...
   do igc = ista , iend
      ...
      kminus = max( kgc-2 , ksta )
      kplus  = min( kgc+2 , kend )

      jminus = max( jgc-2 , jsta )
      jplus  = min( jgc+2 , jend )

      iminus = max( igc-2 , ista )
      iplus  = min( igc+2 , iend )
         ...
         do isearchDist = iminus , iplus
            ...
            if ( InterfaceNodesID( isearchDist, jsearchDist, ksearchDist ) > 0 ) then
               ...
               if ( PNodesList(Ilocal)%NumAsocTetrahedra > 0 ) then
                  ...
                  NI = PNodesList(Ilocal)%NumAsocTetrahedra
                        
                  do K = 1, NI ! loop over the simplices with a vertex at Ilocal
...
end subroutine NeighboursPhiCorrection2