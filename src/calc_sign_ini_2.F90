subroutine calc_sign_ini_2(sgndf_RN,signo_RN,normagrad_RN,epslsm,M_Sphi0)
    ...
    do kRN = kRN_sta,kRN_end
        ...
        if(iRN.ge.li_obs_ia.and.iRN.le.li_obs_ib.and.&
           ...
           kRN.ge.li_obs_ka.and.kRN.le.li_obs_kb) then
            ...
            if(sgndf_RN(iRN,jRN,kRN)*(phiR-phiC).lt.zero.and.&
               (sgndf_RN(iRN,jRN,kRN)*(phiC-phiL)+sgndf_RN(iRN,jRN,kRN)*(phiR-phiC)).lt.zero) then

                dphi_dcsi(iRN,jRN,kRN) = dphi_dcsi_plus

            else if(sgndf_RN(iRN,jRN,kRN)*(phiC-phiL).gt.zero.and.&
                    (sgndf_RN(iRN,jRN,kRN)*(phiR-phiC)+sgndf_RN(iRN,jRN,kRN)*(phiC-phiL)).gt.zero) then
            ...
            if (M_Sphi0==1) then ! s(phi0) Kang
                ...
                if(sgndf_RN(iRN,jRN,kRN).ge.epslsm) then
                    signo_RN(iRN,jRN,kRN) = one
                else if (sgndf_RN(iRN,jRN,kRN).le.(-one)*epslsm) then
                    signo_RN(iRN,jRN,kRN) = -one
                else
                    signo_RN(iRN,jRN,kRN) = sgndf_RN(iRN,jRN,kRN)/epslsm-one/pi*sin(pi*sgndf_RN(iRN,jRN,kRN)/epslsm)
                end if
    
            else ! version original de Carvajal
                signo_RN(iRN,jRN,kRN) = sgndf_RN(iRN,jRN,kRN)/sqrt((sgndf_RN(iRN,jRN,kRN))**2 + (norma_grad_phi *epslsm)**2)
            end if

            ! |grad(phi0)| 
       
            normagrad_RN(iRN,jRN,kRN) = norma_grad_phi
...
!    if(normagrad_RN(iRN,jRN,kRN) > 0.99_rdf) then
!        print *, 'i,j,k : ', iRN, jRN, kRN
!        print *, 'normagrad_RN(iRN,jRN,kRN) = ', normagrad_RN(iRN,jRN,kRN)
!  
!    end if
...
end subroutine calc_sign_ini_2
