...
subroutine ghost_nodes_extrapolation(i,j,k, xs, ys, zs , u_fs_lsqm, pfs, du_dx_fs_lsqm, dp_dx_fs_lsqm)

   implicit none

   integer, intent(in) :: i,j,k
   real (kind = rdf), dimension(1:3), intent(in) :: u_fs_lsqm
   real (kind = rdf)                , intent(in) :: pfs
  ...
  ! I comment this so the routine uses the i_mysta definition from gfm, which doesn't 
  ...
  k_myend = ku - kgp
   ...
   do ii = max( i_mysta , i-extp_swept ) , min( i_myend , i+extp_swept )
      ...
      if ( rsign(ii,jj,kk) < one_half ) then ! (air-phase)
         ...
         pextp =  pfs + dp_dx_fs_lsqm(1) * alpha_local(1) + &
         ...
         ! Normalisation by the global velocity maximum
         if ( limit_ghost_velocities ) then
         
            uextp_norm = norm2( (/uextp , vextp , wextp/) )
            
            if ( uextp_norm > 1E-08 ) then
            
               uextp = ( uextp / uextp_norm ) * min( uextp_norm , max_vel_norm_neighbour )
               vextp = ( vextp / uextp_norm ) * min( uextp_norm , max_vel_norm_neighbour )
               wextp = ( wextp / uextp_norm ) * min( uextp_norm , max_vel_norm_neighbour )
