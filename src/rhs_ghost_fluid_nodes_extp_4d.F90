...
!
!   ! local variables
!   real (kind = rdf), dimension(:) , allocatable :: tmp
!   real (kind = rdf), dimension(:) , allocatable :: var_aux
...
!   nvars = mu-ml+1
!
!   allocate( tmp( 1:nvars ) , var_aux( 1:nvars ) )
!
!   ! Loop boundaries
...
!
!
!         ! skipping nodes within or at the blanking region
!
!         BlankingFlag = .false.
!
!         if ( nblke /= 0 ) then
...
!   deallocate( tmp , var_aux )
   ...
   do i = ilbound , iubound
      ...
      ! using a WLSM
      do kk = max( knearest-2 , ksta ) , min( knearest+2 , kend )
