subroutine NewtonRaphson2( phi, ntetrahedra, TetrahedraList, C_initial_guest, C_i, &
                          TotalWaterVolume )
   ...
   ! input-variables
   ! ------------------
   ! TetrahedraList  : is the list with all the tetrahedrons obtained by the triangulation step
   ...
   parameter(nmax=2000000,MAXITS=15) !200)
   REAL(KIND = RDF):: rst(3)      
   logical::check      
   ...
   call newt( ntetrahedra , TetrahedraList , TotalTetrahedraVolume, &
   ...
   if( its < MAXITS .and. .not.(check)) then
      print *, ' '
      print *, 'NewtonRhapson method was SUCCESFUL!, its = ', its
      print *, ' '
      C_i = C_initial_guest_vec(1)
   else ! I drop the correction
      print *, ' '
      print *, 'NewtonRhapson FAILED --> CCorrected = 0.0'
      print *, ' '
      C_i = zero
   end if

end subroutine NewtonRaphson2
...
SUBROUTINE newt(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, x,n,check,MAXITS,its,itss)
...
!  EXTERNAL fmin
  ...
  call fmin( ntetrahedra , TetrahedraList , TotalTetrahedraVolume , x , fmin_result )
  ...
  do its = 1 , MAXITS
     ...
     call fdjac(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, n,x,fvec,NP,fjac)     
     ...
     call funcv(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, n,x,fvec)
     ...
     call lnsrch(ntetrahedra, TetrahedraList,TotalTetrahedraVolume, n,xold,fold,g,p,x,f,stpmax,check)
...
SUBROUTINE fdjac(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, n,x,fvec,np,df)
...
!    USES funcv
  ...
  do j=1,n
     ...
     x(j)=temp+h
     h=x(j)-temp
     call funcv(ntetrahedra, TetrahedraList, TotalTetrahedraVolume,n,x,f)
     x(j)=temp
     do i=1,n
        df(i,j)=(f(i)-fvec(i))/h
     end do
...
SUBROUTINE fmin(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, x, fmin_result)
...
SUBROUTINE lnsrch(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, n,xold,fold,g,p,x,f,stpmax,check)
...
SUBROUTINE funcv(ntetrahedra, TetrahedraList, TotalTetrahedraVolume, n, C_i, fvec)
   ...
   fvec(1) = TotalTetrahedraVolume - TotalVolumeAux 
