...
contains
  ...
  subroutine init_input_file ()
    ...
    ! if (myid == root) then  ! ELIMINATED IN CORFU.ECE.GATECH.EDU
       ...
       read (1,*) call_solver_daf
       read (1,*) hydraulic_mode
       read (1,*) dynamic_dtau
       read (1,*) non_slip_wall_blanking
       read (1,*) n_save_NSdebug_tsteps
       if(n_save_NSdebug_tsteps > 0) then
         read (1,*) (save_NSdebug_tsteps(tsdbg), tsdbg = 1, n_save_NSdebug_tsteps)
       end if
       ! read blanking data
!       do nz = 1, nzone
...
!                blka = 1 + 2 * (nzb - 1) ! 1 , 3 , 5 ...
!                blkb = 2 + 2 * (nzb - 1) ! 2 , 4 , 6 ...
...
!       end do
       do nz = 1, nzone
          ...
          if (nzblanking(nz) /= 0) then
             ...
             do nzb = 1, nzblanking(nz)
                ...
                ! Calculate column indices for this blanking block
                blka = 1 + 2 * (nzb - 1)  ! First column in pair (1, 3, 5...)
                blkb = blka + 1           ! Second column in pair (2, 4, 6...)
       
                ! Read 6 parameters defining the block type
                ! These populate blktype(1:6, nzb, nz) for current block
                read (1,*) (blktype(n, nzb, nz), n = 1, 6)
       
                ! Read 6 values into rows 1-3 and columns blka/blkb for current zone (nz)
                ! The values are ordered as:
                ! [Row 1, Col blka], [Row 1, Col blkb],
                ! [Row 2, Col blka], [Row 2, Col blkb],
                ! [Row 3, Col blka], [Row 3, Col blkb]
                do dir = 1 , 3 ! dir over i,j,k ranges (each line has a pair of indexes)
                   read (1,*) blanking(dir, blka, nz)  ,  blanking(dir, blkb, nz)
                end do
             end do
          else
             ! If no blanking, set all entries to 0 for this zone
             blanking(:,:,nz) = 0
          end if
      ...
      ! read end of bed-load layer
       ...
       if(n_save_NSdebug_tsteps > 0) then
         current_NSdebug_counter = 1
         current_NSdebug_tstep = save_NSdebug_tsteps(1)
       end if
    ...
    ! call mpi_bcast (delti, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (e_source, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (it_min, 1, mpi_integer, root, mpi_comm_world, ierr)
    ! call mpi_bcast (er_min, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (eo_min, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (iter, ng, mpi_integer, root, mpi_comm_world, ierr)
    ! call mpi_bcast (itm, ng, mpi_integer, root, mpi_comm_world, ierr)
    ! call mpi_bcast (ren, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (cfl1, nzone, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (vnn1, nzone, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (beta, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ...
    ! call mpi_bcast (alfa, irk, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (eps, 4, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (itk, 1, mpi_integer, root, mpi_comm_world, ierr)
    ! call mpi_bcast (cfl2, nzone, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (vnn2, nzone, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ! call mpi_bcast (cdes, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)    
    ! call mpi_bcast (ckin, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)    
    ! call mpi_bcast (cein, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)    
    ! call mpi_bcast (cvin, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)    
    ! call mpi_bcast (ep, 6*nzone, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    !
    ! partc_skip not needed; should eliminate
    !
    ! call mpi_bcast (pdiss_coef, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
    ...
    ! call mpi_bcast (time0, 1, MPI_REAL_TYPE, root, mpi_comm_world, ierr)
  ...
  subroutine init_decomp_3d ()
    ...
    logical :: remain(0:2) ! for 1d communicators creation
    ...
    ! Define MPI_REAL_TYPE precision based on the rdf precision defined
    ! in module precision.F90

    select case ( kind( 1.0_rdf ) )
    ...
    ! Allocate the 1d communicators
    allocate( comm1d_csi ( nzone ) , &
              comm1d_eta ( nzone ) , &
              comm1d_zet ( nzone )   &
            )      
    ...
    ! ---------------------------------------------------------------------------------------------
    ! 1D communicators creation for tridiagonal system solving
    ! (Jorge Sandoval. Jan 2024)

    ! Create subcommunicators and assign two neighboring processes in the ξ-direction.
    remain(0) = .true.
    remain(1) = .false.
    remain(2) = .false.

    call MPI_Cart_sub  ( comm3d(myzone) , remain, comm1d_csi(myzone)%mpi_comm, ierr )
    call MPI_Comm_rank ( comm1d_csi(myzone)%mpi_comm , comm1d_csi(myzone)%myrank , ierr )
    call MPI_Comm_size ( comm1d_csi(myzone)%mpi_comm , comm1d_csi(myzone)%nprocs , ierr )
    call MPI_Cart_shift( comm1d_csi(myzone)%mpi_comm ,0,1, comm1d_csi(myzone)%west_rank , comm1d_csi(myzone)%east_rank , ierr )

    ! Create subcommunicators and assign two neighboring processes in the η-direction
    remain(0) = .false.
    remain(1) = .true.
    remain(2) = .false.

    call MPI_Cart_sub  ( comm3d(myzone), remain, comm1d_eta(myzone)%mpi_comm, ierr )
    call MPI_Comm_rank ( comm1d_eta(myzone)%mpi_comm , comm1d_eta(myzone)%myrank , ierr )
    call MPI_Comm_size ( comm1d_eta(myzone)%mpi_comm , comm1d_eta(myzone)%nprocs , ierr )
    call MPI_Cart_shift( comm1d_eta(myzone)%mpi_comm,0,1, comm1d_eta(myzone)%west_rank , comm1d_eta(myzone)%east_rank , ierr)

    ! Create subcommunicators and assign two neighboring processes in the ζ-direction
    remain(0) = .false.
    remain(1) = .false.
    remain(2) = .true.

    call MPI_Cart_sub  ( comm3d(myzone) , remain , comm1d_zet(myzone)%mpi_comm , ierr)
    call MPI_Comm_rank ( comm1d_zet(myzone)%mpi_comm , comm1d_zet(myzone)%myrank , ierr )
    call MPI_Comm_size ( comm1d_zet(myzone)%mpi_comm , comm1d_zet(myzone)%nprocs , ierr )
    call MPI_Cart_shift( comm1d_zet(myzone)%mpi_comm ,0,1, comm1d_zet(myzone)%west_rank, comm1d_zet(myzone)%east_rank, ierr)
    ...
    !
    myunit = myid + 10 !50
    write (filename, fmt = '(a,i3.3)') 'decofiles/gi.ijkz', myunit
    open (unit = myunit, file = trim(filename), form = 'formatted')
    ...
    !
    ge_ia(:) = gi_ia(:) - igp(:)
    if (myback == mpi_proc_null ) ge_ia(:) = gi_ia(:)
    
    ge_ja(:) = gi_ja(:) - jgp(:)
    if ( myleft == mpi_proc_null ) ge_ja(:) = gi_ja(:) 
    
    ge_ka(:) = gi_ka(:) - kgp(:)
    if ( mydown == mpi_proc_null ) ge_ka(:) = gi_ka(:)

    ge_ib(:) = gi_ib(:) + igp(:)
    if (myfront == mpi_proc_null ) ge_ib(:) = gi_ib(:)
    
    ge_jb(:) = gi_jb(:) + jgp(:)
    if (myright == mpi_proc_null ) ge_jb(:) = gi_jb(:)

    ge_kb(:) = gi_kb(:) + kgp(:)
    if (myup == mpi_proc_null ) ge_kb(:) = gi_kb(:)
...
!             dtau(la:lb),         &
    ...
    allocate(xc(la:lb),           &
			 ...
			 ze(la:lb),			  &
			 zz(la:lb))
	xc = zero; xe = zero; xz = zero
	yc = zero; ye = zero; yz = zero
	zc = zero; ze = zero; zz = zero
...
!     ret = one / ren
  ...
  ! --
   ...
   integer, dimension(:,:), allocatable :: ge_blk_ia , ge_blk_ib , &
                                           ge_blk_ja , ge_blk_jb , &
                                           ge_blk_ka , ge_blk_kb 

   allocate ( gi_blk_ia ( ng , nba_max ) , gi_blk_ib ( ng , nba_max ) , &
              gi_blk_ja ( ng , nba_max ) , gi_blk_jb ( ng , nba_max ) , &
              gi_blk_ka ( ng , nba_max ) , gi_blk_kb ( ng , nba_max )  )

   allocate ( ge_blk_ia ( ng , nba_max ) , ge_blk_ib ( ng , nba_max ) , &
              ge_blk_ja ( ng , nba_max ) , ge_blk_jb ( ng , nba_max ) , &
              ge_blk_ka ( ng , nba_max ) , ge_blk_kb ( ng , nba_max )  )
   ...
   if ( nzblanking(myzone) /= 0 ) then
      ...
      do nzb = 1 , nzblanking(myzone)
         ...
         if ( i_in .and. j_in .and. k_in ) then
            ...
            gi_blk_ia ( 1 , nblk ) = blk_ia  ;  gi_blk_ib ( 1 , nblk ) = blk_ib
            gi_blk_ja ( 1 , nblk ) = blk_ja  ;  gi_blk_jb ( 1 , nblk ) = blk_jb
            gi_blk_ka ( 1 , nblk ) = blk_ka  ;  gi_blk_kb ( 1 , nblk ) = blk_kb
            ...
            do n = 2, ng
               ...
               if ( blktype( 2 , nzb , myzone ) /= 0 ) then
               ...
               if ( blktype( 4 , nzb , myzone ) /= 0 ) then
               ...
               if ( blktype( 6 , nzb , myzone ) /= 0 ) then
         ...
         end if ! i_in .and. j_in .and. k_in 

      end do ! do nzb = 1 , nzblanking(myzone)

      ! Blanking areas considering exterior nodes
      nblke = 0
      ...
      do nzb = 1 , nzblanking(myzone)
         ...
         ! i inside ?
         if (ge_ia(1) <= blanking( 1 , blka , myzone ) .and. &
                         blanking( 1 , blka , myzone ) <= ge_ib(1) ) i_in = .true.
          
         if (ge_ia(1) <= blanking( 1 , blkb , myzone ) .and. &
                         blanking( 1 , blkb , myzone ) <= ge_ib(1) ) i_in = .true.
         
         if (ge_ia(1) >= blanking( 1 , blka , myzone ) .and. &
                         blanking( 1 , blkb , myzone ) >= ge_ib(1) ) i_in = .true.
         
         ! j inside ?
         if (ge_ja(1) <= blanking( 2 , blka , myzone ) .and. &
                         blanking( 2 , blka , myzone ) <= ge_jb(1) ) j_in = .true.
          
         if (ge_ja(1) <= blanking( 2 , blkb , myzone ) .and. &
                         blanking( 2 , blkb , myzone ) <= ge_jb(1) ) j_in = .true.
          
         if (gi_ja(1) >= blanking( 2 , blka , myzone ) .and. &
                         blanking( 2 , blkb , myzone ) >= gi_jb(1) ) j_in = .true.
         
         ! k inside ?
         if (ge_ka(1) <= blanking( 3 , blka , myzone ) .and. &
                         blanking( 3 , blka , myzone ) <= ge_kb(1) ) k_in = .true.
          
         if (ge_ka(1) <= blanking( 3 , blkb , myzone ) .and. &
                         blanking( 3 , blkb , myzone ) <= ge_kb(1) ) k_in = .true.
         ...
         if ( i_in .and. j_in .and. k_in ) then
            ...
            blk_ia = max( ge_ia(1) , blanking( 1 , blka , myzone ) ) 
            blk_ib = min( ge_ib(1) , blanking( 1 , blkb , myzone ) )

            blk_ja = max( ge_ja(1) , blanking( 2 , blka , myzone ) )
            blk_jb = min( ge_jb(1) , blanking( 2 , blkb , myzone ) )
      ...
      allocate ( li_blk_ia( ng , nblk ) , li_blk_ib( ng , nblk ) , &
                 li_blk_ja( ng , nblk ) , li_blk_jb( ng , nblk ) , &
                 li_blk_ka( ng , nblk ) , li_blk_kb( ng , nblk )  )


      allocate ( le_blk_ia( ng , nblke ) , le_blk_ib( ng , nblke ) , &
                 le_blk_ja( ng , nblke ) , le_blk_jb( ng , nblke ) , &
                 le_blk_ka( ng , nblke ) , le_blk_kb( ng , nblke )  )
...
!#endif
    ...
    close(unit = myunit)
      ...
      do n  = 1 , ng   
         ...
         ! ghost nodes
         ! I substract the interior to get -1,0 as ghost nodes indexes
         le_blk_ia( n , nb ) = ge_blk_ia( n , nb ) - gi_ia(n) + 1 
         le_blk_ib( n , nb ) = ge_blk_ib( n , nb ) - gi_ia(n) + 1

         le_blk_ja( n , nb ) = ge_blk_ja( n , nb ) - gi_ja(n) + 1
         le_blk_jb( n , nb ) = ge_blk_jb( n , nb ) - gi_ja(n) + 1
         
         le_blk_ka( n , nb ) = ge_blk_ka( n , nb ) - gi_ka(n) + 1
         le_blk_kb( n , nb ) = ge_blk_kb( n , nb ) - gi_ka(n) + 1


      end do ! nb = 1 , nblke
      end do ! n  = 1 , ng  

   end if ! nzblanking(myzone) /= 0

   deallocate ( gi_blk_ia , gi_blk_ib , &
                gi_blk_ja , gi_blk_jb , &
                gi_blk_ka , gi_blk_kb  )

   deallocate ( ge_blk_ia , ge_blk_ib , &
                ge_blk_ja , ge_blk_jb , &
                ge_blk_ka , ge_blk_kb  )
  ...
  subroutine init_osg_interface ()
    ...
    call mpi_bcast (hostst, 4*n_int_total, MPI_INTEGER, root, &
                    mpi_comm_world, ierr)
    call mpi_bcast (coeft,  3*n_int_total, MPI_REAL_TYPE,    root, &
                    mpi_comm_world, ierr)
...
!     close (unit = myunit4)
  ...
  subroutine init_outlet_bc ()
    ...
    integer :: kmax
    integer :: ijkmax
    integer :: idxi , idxe ! index ini and end
    ...
    if ( ijkmax == imax ) then
        ! I make sure idxi starts at -1 to avoid outbounding afterwards
        idxi = min ( le_ia(1) , 1 - max( igp(1) , jgp(1) , kgp(1) ) )
        idxe = le_ib(1)
    else if ( ijkmax == jmax ) then
        idxi = min ( le_ja(1) , 1 - max( igp(1) , jgp(1) , kgp(1) ) )
        idxe = le_jb(1)
    else
        idxi = min ( le_ka(1) , 1 - max( igp(1) , jgp(1) , kgp(1) ) )
        idxe = le_kb(1)
    end if

    !allocate (rat(ijkmax,ijkmax,6))
    allocate ( rat( idxi : idxe , idxi : idxe , 6 ) )

    !if(myid ==0) print *, ijkmax
    ! csi direction
    b = 1
    ...
    if (btype(b,myzone) == 4 .or. btype(b,myzone) == 5) then
       ...
       do j = le_ja(1), le_jb(1)
          ...
          if (bdir(b,myzone) == 1 .and. abs(x(l2) - x(l1)) > eps_sims ) &
              rat(j,k,b) = (x(l1) - x(l0)) / (x(l2) - x(l1))
          if (bdir(b,myzone) == 2 .and. abs(y(l2) - y(l1)) > eps_sims ) &
              rat(j,k,b) = (y(l1) - y(l0)) / (y(l2) - y(l1))
          if (bdir(b,myzone) == 3 .and. abs(z(l2) - z(l1)) > eps_sims ) &
    ...
    if (btype(b,myzone) == 4 .or. btype(b,myzone) == 5) then
       ...
       do j = le_ja(1), le_jb(1)
          ...
          if (bdir(b,myzone) == 1 .and. abs(x(l1) - x(l2)) > eps_sims ) &
              rat(j,k,b) = (x(l0) - x(l1)) / (x(l1) - x(l2))
          if (bdir(b,myzone) == 2 .and. abs(y(l1) - y(l2)) > eps_sims ) &
              rat(j,k,b) = (y(l0) - y(l1)) / (y(l1) - y(l2))
          if (bdir(b,myzone) == 3 .and. abs(z(l1) - z(l2)) > eps_sims ) &
