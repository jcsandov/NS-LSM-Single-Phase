program postproc_debug_single_file
   ...
   !
   ! gfortran -o debug2tec postproc.debug_single_file.f90 tecio64.a f2kcli.f90 str_int.f90 -lstdc++
   ! 
   ! * (f2kcli.f90 and str_int.f90 must be in the same folder where compilation is made)
   ! ** (tecio64.a might be replaced by libtecio.a if we are compiling in OSX architecture)
   !
   ! INPUTS
   ! ---------
   ! 
   ...
   ! 6. debugnames.txt (in the same folder of this routine) --> to obtain the total amount of debugged 
   !                                                            variables and their names
   !
   ! RUNNING
   ! ---------
   ! 
   ! ./debug2tec nproc ntimeStart ntimeEnd ntimeSkip
   !
   ...
   !
   ! Jorge Sandoval, UoE/PUC. Edinburgh, December 31th, 2021.
   ...
   character (len = 256), dimension(40) :: debugnames
   integer :: ideb
...
subroutine process_comb_files_debug (ts)
   ...
   real (kind = rdf), dimension(:,:,:,:,:), allocatable :: vardebug_master
   character (len = 2048) :: tecvarnames
   ...
   if ( pressureflag ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'P'    , ','
   if ( uflag        ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'U'    , ','
   if ( vflag        ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'V'    , ','
   if ( wflag        ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'W'    , ','
   if ( nutflag      ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'NU-T' , ','
   if ( phiflag      ) write(tecvarnames,fmt = '(3a)') trim(tecvarnames) , 'PHI'  , ','

   ! concatenation loop: we format and concatenate all the variables name from debugnames.txt
   if( numdebugvars > 0) then
      do ideb = 1, numdebugvars
         write( tecvarnames , fmt = '(3a)' ) trim( tecvarnames ) , trim( debugnames( ideb ) ) , ','
      end do
   end if

   ! remove undesired commas at the end of the string if they exist
   tecvarnames = remove_comma(tecvarnames)

   I = TecIni('debugfile'//NULLCHR,    &   ! title of file
     'X, Y, Z, ' // trim(tecvarnames) //NULLCHR,   &   ! list of variables
     trim(filename)//'.plt'//NULLCHR, &   ! output file name
     '.'//NULLCHR,                 &
     Debug,                        &
     VIsDouble)
   ...
   do nz = 1, nzone
      ...
      III = km(nz) * jm(nz) * im(nz)
                         ...
                         I   = TecDat( III , x(  1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
                         I   = TecDat( III , y(  1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
                         I   = TecDat( III , z(  1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      if( pressureflag ) I   = TecDat( III , q(1,1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      if( uflag        ) I   = TecDat( III , q(2,1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      if( vflag        ) I   = TecDat( III , q(3,1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      if( wflag        ) I   = TecDat( III , q(4,1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      if (nutflag      ) I   = TecDat( III , nu(1:im(nz) ,1:jm(nz),1:km(nz),nz) , 1 )
      if (phiflag      ) I   = TecDat( III , phi(1:im(nz),1:jm(nz),1:km(nz),nz) , 1 )
      
      ! writing the debug variables
      
      if( numdebugvars > 0) then      
         do ideb = 1, numdebugvars
            I   = TecDat(III,vardebug_master(ideb,1:im(nz),1:jm(nz),1:km(nz),nz),1)
         end do      
      end if
   end do !nz
...
subroutine read_input_file( )
   ...
   ! Open input file
   open(unit=10, file='debugnames.txt', status='old', action='read', iostat=ierr)
...
end program postproc_debug_single_file
